...
import { Sofa, Loader2, Check, X, Download, Video } from "lucide-react";
...
export default function StudioVirtualStaging({ photos: projectPhotos, onPhotoReplaced, onAddPhoto, onPhotoDeleted, projectId }) {
  ...
  } = usePhotoWorkState(allPhotos, 0.40);
  ...
  const runStaging = async () => {
    ...
    setResultPhoto(null);
    try {
      const s3Url = await generateFalImage(originalPhoto, prompt, strength, "virtual-staging");
