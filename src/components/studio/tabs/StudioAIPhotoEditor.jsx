...
export default function StudioAIPhotoEditor({ photos: projectPhotos, onPhotoReplaced }) {
  ...
  const currentPhoto = appliedEdits[selectedIdx] || allPhotos[selectedIdx];
  ...
  const downloadPhoto = (url, filename) => {
    ...
    a.href = url;
    a.download = filename || "edited-photo.jpg";
    a.target = "_blank";
    a.click();
  };
          ...
          <div className="bg-white rounded-2xl border border-gray-100 overflow-hidden">
            <div className="flex items-center justify-between px-4 pt-3 pb-2">
              ...
              {(resultPhoto || appliedEdits[selectedIdx]) && (
                <button onClick={() => downloadPhoto(resultPhoto || appliedEdits[selectedIdx], `edited-${selectedIdx + 1}.jpg`)}
            ...
            {AI_EDITS.filter(e => e.key !== "custom").map(edit => (
              <button key={edit.key} onClick={() => setSelectedEdit(edit.key)}
                className={`text-xs font-medium rounded-xl px-3 py-2.5 border-2 text-left transition-all leading-tight ${
                  selectedEdit === edit.key ? "border-purple-700 bg-purple-50 text-purple-800" : "border-gray-100 bg-gray-50 text-gray-600 hover:border-gray-300"
                }`}>
                {edit.label}
              </button>
            ))}
