...
import { Play, Loader2, CheckCircle2, ExternalLink, RefreshCw, Download, AlertCircle } from "lucide-react";
...
export default function CreatomateRender({ project, voiceoverUrl, musicUrl, selectedBrandKit }) {
  ...
  const [apiKey] = useState(() => localStorage.getItem("creatomate_api_key") || "");
  ...
  const handleRender = async () => {
    if (!apiKey) { toast({ title: "Creatomate API key not set. Add it in Settings → Integrations.", variant: "destructive" }); return; }
